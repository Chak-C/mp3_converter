...
def toggle_advanced_properties(widget, properties):
    ...
    if widget['advanced_properties_var'].get():
        ...
        widget['bottom_frame'].pack()
    else:
        widget['additional_properties_frame'].pack_forget()
        widget['bottom_frame'].pack_forget()
...
def search(mediator: Type[Mediator], widgets):
    """
    Param:
        Mediator for url to update all pages
        widgets (dictionary): must contain ["Title"] and ["Album Artist"]
            (yes, im lazy and not handling the errors for this)
    Return:
        mediator will update all widgets of the searched url
    """
    title = widgets['Title'].get() if widgets['Title'].get() else ''
    artist = widgets['Album Artist'].get() if widgets['Album Artist'].get() else ''
    url = af.search_youtube(title,artist)

    if url == 'No video found': messagebox.showwarning("Warning", f"No results for [track - {title}, artist - {artist}")
    else:
        mediator.set_item(url)
    pass
